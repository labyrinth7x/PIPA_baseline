...
def get_network(args):
    ...
    flatten_output = all_layers['flatten_output']
    softmax_label = mx.sym.Variable('softmax_label')
    net = mx.sym.Dropout(data = flatten_output,p = 0.5)
    weight_net_init = mx.init.Xavier()
    #weight_512 = mx.sym.Variable('embed512_weight', init = weight_net_init, shape = (512,1024), lr_mult = 1.0)
    #net = mx.sym.FullyConnected(data = net, weight = weight_512, num_hidden = 512, name = 'embed512', no_bias = True)
    #net = mx.sym.Activation(data = net, act_type = 'relu', name = 'activation512')
    #net = mx.sym.Dropout(data = net, p = 0.5)
    weight_256 = mx.sym.Variable('embed_weight', init= weight_net_init, shape = (args.embed_size,1024), lr_mult = args.lr_mult)
...
def add_data_aug_args(parser):                                                                                                          
    ...
    aug.add_argument('--random-crop', type=int, default=0,help='if or not randomly crop the image')
    ...
    aug.add_argument('--min_shorter_edge', type = int, default = 0)
    aug.add_argument('--max_shorter_edge', type = int, default = 0)
    return aug     
...
def parse_args():
    ...
    parser.add_argument('--epoch_load', type = int, default = 126) 
    ...
    parser.add_argument('--region', type = str, help = 'train region')
    parser.add_argument('--lr_mult', type =float, default = 3.0)
...
def record(args):
    ...
    record.write('train root:' + args.train_root + '\n')
    ...
    # save logs to train.log
    # logging.basicConfig(filename = folder + '/train.log', level = logging.INFO, filemode = 'w', format = '%(message)s')
